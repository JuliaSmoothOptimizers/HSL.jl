...
run(
  `./configure --prefix=$usrdir FC=$(HSL_FC) F77=$(HSL_F77) CC=$(HSL_CC) CFLAGS="-O3 -fPIC" FFLAGS="-O3 -fPIC -fopenmp" FCFLAGS="-03 -fPIC -fopenmp" --with-blas="-L$libblas_dir $libblas" --with-metis="-L$libmetis_dir -lmetis"`,
)
run(`make install`)
cd(@__DIR__)
