...
# libhsl_ma97 = library_dependency("libhsl_ma97", depends=[libopenblas, libmetis4])
...
provides(SimpleBuild,
         (@build_steps begin
            ...
            (@build_steps begin
              ...
              `./configure F77=gfortran CFLAGS=-fPIC FFLAGS="-fPIC -fopenmp" FCFLAGS="-fPIC -fopenmp" --prefix=$hsl_ma97_prefix --with-blas="$blas_lib" --with-lapack="$lapack_lib" --with-metis="$metis_libpath"`
              `make install`
              `gfortran -fPIC -shared -Wl,$all_load $hsl_ma97_libdir/libhsl_ma97.a $(split(blas_lib)) $(split(lapack_lib)) $(split(metis_libpath)) -lgomp -Wl,$noall_load -o $hsl_ma97_libdir/libhsl_ma97.$so`
            end)
